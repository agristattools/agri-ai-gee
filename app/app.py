"""
Agri-AI-GxE: Genotype x Environment Prediction Dashboard
...
st.set_page_config(page_title="Agri-AI-GxE | Yield Predictor", page_icon="", layout="wide")

st.title("AI-Driven Genotype x Environment Yield Predictor")
st.markdown("**Hybrid System:** Classical Breeding Statistics (BLUP/AMMI) + Machine Learning (XGBoost)")
st.divider()
...
mode = st.sidebar.radio("", ["Explore SoyURT Data", "Upload My Own Data"])
st.sidebar.divider()

if mode == "Explore SoyURT Data":
    st.sidebar.subheader("Prediction Controls")
    ...
    env_mean_yield = st.sidebar.slider("Expected Env Mean Yield", 20.0, 100.0, float(geno_data['Env_Mean_Yield']) if not pd.isna(geno_data['Env_Mean_Yield']) else 55.0, 1.0)
    ...
    with col1:
        ...
        if prediction > overall_mean:
            ...
            st.warning(f"Below average ({(prediction - overall_mean):.1f} bu/ac)")
    with col2:
        st.subheader("Genotype Profile")
        fig, ax = plt.subplots(figsize=(6, 6), subplot_kw=dict(projection='polar'))
        metrics = ['BLUP', 'Mean Yield', 'Stability']
        values = [geno_data['BLUP'], geno_data['Genotype_Mean_Yield'], max(0, 10 - geno_data['Stability']) * 5]
    ...
    tab1, tab2 = st.tabs(["Top Genotypes", "GxE Visualization"])
    with tab1:
        top_genos = df_train.groupby('G').agg({'BLUP': 'first', 'Genotype_Mean_Yield': 'first', 'Stability': 'first'}).sort_values('BLUP', ascending=False).head(10).reset_index()
    ...
    st.sidebar.markdown("Required: Genotype, latitude, longitude\nOptional: altitude, year, yield")
    st.sidebar.divider()
    ...
    if uploaded_file is not None:
        ...
        if missing:
            ...
            if 'altitude' not in user_df.columns:
                ...
                st.info("altitude not found - using default: 300m")
            if 'year' not in user_df.columns:
                user_df['year'] = 2025
                st.info("year not found - using default: 2025")
            ...
            with col1:
                ...
                ax.axvline(user_df['Predicted_Yield_bu_ac'].mean(), color='red', linestyle='--', label=f"Mean: {user_df['Predicted_Yield_bu_ac'].mean():.1f}")
            ...
            with col2:
                if 'yield' in user_df.columns:
                    ...
                    lims = [min(user_df[['yield', 'Predicted_Yield_bu_ac']].min()), max(user_df[['yield', 'Predicted_Yield_bu_ac']].max())]
            ...
            st.download_button(label="Download Predictions (CSV)", data=csv, file_name='predicted_yields.csv', mime='text/csv')
...
st.divider()
st.markdown("---")
st.markdown("**Zulqarnain (2026).** *AI-Integrated Framework for GxE Modeling.* [GitHub](https://github.com/agristattools/agri-ai-gee) | Data: USDA SoyURT (CC BY 4.0)")